...
│   │   │   ├── *.mdx
│   │   │   └── es/*.mdx
│   │   ├── writing/         # Essays (en + es/ pairs MUST share filenames
│   │   │   ├── *.mdx        #   so /writing/<slug>/ and /es/writing/<slug>/
│   │   │   └── es/*.mdx     #   hreflang pairs resolve; draft: true hides)
...
6. **writing** - Essays (pillars: regulated-ai, silicon-to-software, founder-journey; `draft: true` excludes an essay from pages, RSS, sitemap, llms.txt, and OG generation; all access goes through `src/lib/writing.ts`; per-essay OG cards are generated by `scripts/generate-og.mjs` and committed because CI bypasses prebuild)
7. **personal** - Personal values and passions
